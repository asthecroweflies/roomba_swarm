...
/*    struct serverArgs *sa;
    if (argc != 2) {
        printf("Usage: ./server.out <number of clients>\n");
        exit(-1);
    }
    int totalClients = atoi(argv[1]);
    if (!((expectedClients > 1) && (expectedClients < 5))){
        printf("Usage: ./server.out <number of clients>\n");
        exit(-1);
    }

    sa->expectedClients = totalClients;*/
    ...
    */
    ...
    while (1)
    {
        ...
        if (connectedClients == numClients) {
            while (1) {
                char roombaCmd[128];
                printf("Enter roomba command sequence: \n");
                int charCount;
                for (charCount = 0; charCount < sizeof(roombaCmd) / sizeof(roombaCmd[0]); charCount++) {
                    if (!isalpha(roombaCmd[charCount])) {
                        break;
                    }
                }
                roombaCmd[charCount] = '\0';

                fgets(roombaCmd, 128, stdin);
                strncpy(wa->moveCmd, roombaCmd, sizeof(roombaCmd));
                int iter;
                for (iter = 0; iter < connectedClients; iter++) {
                    send(clientSockets[iter], roombaCmd, strlen(roombaCmd), 0);
                    //printf("clientSockets[%d] = %d", iter, clientSockets[iter]);
                    //wa->socket = clientSockets[iter];
                    //pthread_create(&threads[iter], NULL, service_single_client, wa);
                }
            }
            //if (pthread_create(&worker_thread, NULL, service_single_client, wa) != 0) 
            
            /*if (pthread_create(&threads[tid++], NULL, service_single_client, wa) != 0)
            {
...
void *service_single_client(void *args) {
    ...
    socket = wa->socket;
    strncpy(tosend, wa->moveCmd, sizeof(wa->moveCmd));
